...
export function ActivityPanel({ issueId, canComment, canInternal, currentUserId }: ActivityPanelProps) {
  ...
  return (
    ...
    >
      {canComment && (
        <CommentComposer issueId={issueId} canInternal={canInternal} borderColor={PANEL_BORDER} />
      )}

      {/* Pinned header */}
      <div className="shrink-0 px-4 pt-3 pb-2.5 border-b space-y-2" style={{ borderColor: PANEL_BORDER }}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-36">
                  ...
                  {(["day", "type", "user", "none"] as ActGroup[]).map(g => (
                    <DropdownMenuItem key={g} onClick={() => setGroup(g)} className="justify-between text-xs">
                      {GROUP_LABELS[g]}
              ...
              <div key={label}>
                ...
                <div className="space-y-3">
                  {items.map(item => {
                    ...
                    return                               <AttachmentRow key={item.id} item={item}                           isSelf={isSelf} issueId={issueId} />;
                  })}
                </div>
              </div>
            ))}
