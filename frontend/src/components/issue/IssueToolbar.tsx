...
import { ArrowUpDown, Bookmark, Check, Filter, Layers, Loader2, X } from "lucide-react";
...
import type { SortField, SortDir, ExtraFilters } from "@/hooks/use-issue-list-state";
...
interface Props {
  ...
  setExtraFilter: <K extends keyof ExtraFilters>(key: K, val: ExtraFilters[K]) => void;
...
const STATUS_OPTIONS:   IssueStatus[]   = ["new", "in_progress", "on_hold", "resolved", "closed", "cancelled"];
const PRIORITY_OPTIONS: PriorityLevel[] = ["critical", "high", "moderate", "low"];
const TYPE_OPTIONS:     IssueType[]     = ["bug", "feature_request", "question", "incident"];
...
export function IssueToolbar({
  search, setSearch,
  extraFilters, setExtraFilter, setExtraFilters, extraFilterCount,
  sortField, sortDir, cycleSort,
  groupBy, setGroupBy,
  activeViewId, activeViewLabel,
  saveViewOpen, setSaveViewOpen,
  saveViewName, setSaveViewName,
  saveNameError, setSaveNameError,
  saveViewInputRef, handleSaveView,
  createSavedViewMutation,
  deleteSavedView, deleteSavedViewMutation,
}: Props) {
  return (
    <div className="flex flex-col gap-1.5 px-4 py-2 border-b bg-background shrink-0">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className={cn("h-7 text-xs gap-1.5", extraFilterCount > 0 && "border-primary text-primary")}
          >
        ...
        <DropdownMenuContent align="start" className="w-48">
          <DropdownMenuLabel className="text-xs py-1.5">Status</DropdownMenuLabel>
          <DropdownMenuSeparator />
          {STATUS_OPTIONS.map(st => (
            <DropdownMenuItem key={st} onClick={() => setExtraFilter("status", st)} className="justify-between text-xs">
              <span className="capitalize">{st.replace(/_/g, " ")}</span>
              {extraFilters.status === st && <Check className="size-3 ml-2" />}
            </DropdownMenuItem>
          ))}
          <DropdownMenuSeparator />
          <DropdownMenuLabel className="text-xs py-1.5">Priority</DropdownMenuLabel>
          <DropdownMenuSeparator />
          {PRIORITY_OPTIONS.map(p => (
            <DropdownMenuItem key={p} onClick={() => setExtraFilter("priority", p)} className="justify-between text-xs capitalize">
              {p}
              {extraFilters.priority === p && <Check className="size-3 ml-2" />}
            </DropdownMenuItem>
          ))}
          <DropdownMenuSeparator />
          <DropdownMenuLabel className="text-xs py-1.5">Type</DropdownMenuLabel>
          <DropdownMenuSeparator />
          {TYPE_OPTIONS.map(t => (
            <DropdownMenuItem key={t} onClick={() => setExtraFilter("type", t)} className="justify-between text-xs">
          ...
          {extraFilterCount > 0 && <>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => setExtraFilters({})} className="text-xs text-destructive justify-center">
              Clear filters
            </DropdownMenuItem>
          </>}
        </DropdownMenuContent>
      ...
      {extraFilters.status && (
        <span className="inline-flex items-center gap-1 rounded-md border border-primary/40 bg-primary/5 px-2 h-7 text-xs text-primary">
          <span className="capitalize">{extraFilters.status.replace(/_/g, " ")}</span>
          <button onClick={() => setExtraFilter("status", extraFilters.status)} className="hover:text-destructive transition-colors">
            <X className="size-3" />
          </button>
        </span>
      )}
      {extraFilters.priority && (
        <span className="inline-flex items-center gap-1 rounded-md border border-primary/40 bg-primary/5 px-2 h-7 text-xs text-primary capitalize">
          {extraFilters.priority}
          <button onClick={() => setExtraFilter("priority", extraFilters.priority)} className="hover:text-destructive transition-colors">
            <X className="size-3" />
          </button>
        </span>
      )}
      {extraFilters.type && (
        <span className="inline-flex items-center gap-1 rounded-md border border-primary/40 bg-primary/5 px-2 h-7 text-xs text-primary">
          <span className="capitalize">{extraFilters.type.replace(/_/g, " ")}</span>
          <button onClick={() => setExtraFilter("type", extraFilters.type)} className="hover:text-destructive transition-colors">
            <X className="size-3" />
          </button>
        </span>
      )}
      ...
      {(extraFilterCount > 0 || !!search) && (
        <button
          onClick={() => { setExtraFilters({}); setSearch(""); }}
      ...
      <DropdownMenu>
        <div className="flex items-center">
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              className={cn("h-7 text-xs gap-1.5", groupBy && "rounded-r-none border-r-0")}
            >
        ...
        <DropdownMenuContent align="start" className="w-40">
          ...
          {["status", "priority", "product", "assignee"].map(f => (
            <DropdownMenuItem key={f} onClick={() => setGroupBy(f)} className="justify-between text-xs capitalize">
              {f} {groupBy === f && "✓"}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      ...
      {saveViewOpen ? (
        <div className="flex items-center gap-1">
          <div className="flex flex-col">
            <input
              ...
              value={saveViewName}
              onChange={e => { setSaveViewName(e.target.value); setSaveNameError(""); }}
              onKeyDown={e => {
                if (e.key === "Enter")  handleSaveView();
                if (e.key === "Escape") { setSaveViewOpen(false); setSaveViewName(""); setSaveNameError(""); }
              }}
              placeholder="Name this view…"
              className={cn(
                "h-7 w-36 rounded-md border bg-background px-2 text-xs outline-none focus:ring-1 focus:ring-ring",
                saveNameError ? "border-destructive focus:ring-destructive" : "border-input",
              )}
            />
            {saveNameError && (
              <span className="mt-0.5 text-[10px] text-destructive">{saveNameError}</span>
            )}
          </div>
          <Button
            size="sm"
            ...
            disabled={!saveViewName.trim() || createSavedViewMutation.isPending}
          >
          ...
          <Button
            ...
            size="sm"
            className="h-7 w-7 p-0"
            onClick={() => { setSaveViewOpen(false); setSaveViewName(""); setSaveNameError(""); }}
          >
            <X className="size-3" />
          </Button>
        </div>
        ...
        <ConfirmDialog
          trigger={
            <Button variant="outline" size="sm" className="h-7 text-xs gap-1.5" title="Remove this saved view">
