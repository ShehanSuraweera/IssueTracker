...
export function useIssueListState() {
  ...
  const [activeTab, setActiveTab] = useState<"default" | "my">(() => {
    ...
    return view.startsWith("sv:")
      ? "my"
      : ((localStorage.getItem("issues-active-tab") as "default" | "my") ??
          "default");
  });
  const [activeViewId, setActiveViewId] = useState(
    () => searchParams.get("view") ?? "all",
  );
  const [activeViewQuery, setActiveViewQuery] = useState<
    Partial<ListIssuesQuery>
  >({});
  ...
  const [pinned, setPinned] = useState<Set<string>>(() => {
    try {
      return new Set(
        JSON.parse(localStorage.getItem("sidebar-pinned") ?? "[]"),
      );
    } catch {
      return new Set();
    }
  ...
  const setExtraFilter = <K extends keyof ExtraFilters>(
    key: K,
    val: ExtraFilters[K],
  ) => {
    deactivateSavedView();
  ...
  const handleSetExtraFilters = (f: ExtraFilters) => {
    deactivateSavedView();
    setExtraFilters(f);
  };
  const handleSetGroupBy = (g: string | null) => {
    deactivateSavedView();
    setGroupBy(g);
  };
  ...
  const handleSaveView = () => {
    ...
    setSaveNameError("");
    const query: Omit<ListIssuesQuery, "page" | "limit"> = {
      ...activeViewQuery,
    };
  ...
  const commitRename = (viewId: string) => {
    ...
    if (
      savedViews.some(
        (v) => v.name.toLowerCase() === name.toLowerCase() && v.id !== viewId,
      )
    ) {
  ...
  const handleExport = async (format: "csv" | "json") => {
    ...
    try {
      const params = {
        ...
        ...(activeViewQuery.product_id
          ? { product_id: activeViewQuery.product_id }
          : {}),
      };
  ...
  useEffect(() => {
    localStorage.setItem("issues-active-tab", activeTab);
  }, [activeTab]);
  ...
  const {
  ...
  } = useInfiniteIssues(
    {
      ...activeViewQuery,
      ...extraFilters,
      sort: `${sortField}_${sortDir}` as NonNullable<
        typeof activeViewQuery.sort
      >,
    },
    debouncedSearch,
